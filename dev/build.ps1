param([switch]$ConfigureOnly, [switch]$Fresh, [switch]$BuildOnly, [switch]$WithoutBots, [int]$Workers=6)
...
if($Bots){$build=Join-Path $source 'build/local';$module='static'}
if($CoreOnlyInModuleBuild){$build=Join-Path $source 'build/local';$module='disabled'}
$taskBinaryDirectory=Join-Path $build 'bin'
$taskConfigureOptions=@()
if($Fresh){$taskConfigureOptions+='--fresh'}
& $cmake @taskConfigureOptions -S $source -B $build -G Ninja "-DCMAKE_MAKE_PROGRAM=$ninja" "-DCMAKE_PREFIX_PATH=$deps" "-DBoost_ROOT=$deps" "-DBoost_DIR=$deps/share/boost" "-DACE_ROOT=$deps" -DCMAKE_BUILD_TYPE=Release -DBUILD_PLAYERBOTS=OFF -DBUILD_ELUNA=OFF -DENABLE_SOAP=ON -DMODULES=disabled "-DMODULE_TORTOISEBOTS=$module" -DMODULE_MOD_PLAYERBOTS=disabled -DUSE_PCH=ON -DUSE_PCH_OLD=ON -DUSE_DISCORD_BOT=OFF -DUSE_EXTRACTORS=OFF "-DENABLE_LTO=$lto" "-DCMAKE_INSTALL_PREFIX=$output" "-DCMAKE_RUNTIME_OUTPUT_DIRECTORY=$taskBinaryDirectory"
...
if(-not $ConfigureOnly -and -not $BuildOnly){
    ...
    foreach($taskConfig in @(@('ahbot','modules/TortoiseBots/ahbot/ahbot.conf.dist.in'), @('aiplayerbot','modules/TortoiseBots/ai/playerbot/aiplayerbot.conf.dist.in'))){
